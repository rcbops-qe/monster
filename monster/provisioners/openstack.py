...
class Openstack(Provisioner):
    ...
    def provision(self, template, deployment):
        ...
        :rtype: list
        """
        self.logger.info("Provisioning in the cloud!")
        # acquire connection
    ...
    def chef_instance(self, deployment, name, flavor="2GBP"):
        ...
        while not run_cmd(command)['success']:
            self.logger.warning("Epic failure. Retrying...")
            sleep(1)
    ...
    def build_instance(self, name="server", image="ubuntu",
        ...
        self.logger.info("Building:{0}".format(name))
        server = self.wait_for_state(self.compute_client.servers.get, server,
                                     "status", ["ACTIVE", "ERROR"])
        if server.status == "ERROR":
            self.logger.error("Instance entered error state. Retrying...")
            server.delete()
        ...
        while not sshup:
            try:
                ...
                sshup = False
                self.logger.debug("Waiting for ssh connection...")
                sleep(1)
    ...
    def _client_search(self, collection_fun, attr, desired, attempts=None,
        ...
        while in_attempt(attempt):
            try:
                ...
                self.logger.error("Wait: Request error:{0}-{1}".
                                  format(desired, e))
                continue
        get_attr = lambda x: getattr(x, attr)
        self.logger.debug("Search:{0} for {1} in {2}".format(
    ...
    def wait_for_state(self, fun, obj, attr, desired, interval=15,
        ...
        while getattr(obj, attr) not in desired and in_attempt(attempt):
            self.logger.info("Waiting:{0} {1}:{2}".format(obj, attr,
                                                          getattr(obj, attr)))
            sleep(interval)
            obj = fun(obj.id)
            attempt = attempt + 1
