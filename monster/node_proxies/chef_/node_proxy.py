...
import monster.util
import monster.node_proxies.base as base
...
class NodeProxy(base.NodeProxy):
    """Provides chef_ related server functions."""
    def __init__(self, name, ip, user, password, product, deployment,
                 provisioner, environment, branch, run_list=None):

        super(NodeProxy, self).__init__(name, ip, user, password, product,
    ...
    def build(self):
        """Builds the node."""
        self.clear_run_list()
        super(NodeProxy, self).build()

    def upgrade(self, times=1, accept_failure=False):
        """Upgrade the node according to its features.
        :param times: number of times to run chef_-client
        :type times: int
        :param accept_failure: whether to accept failure of chef_-client runs
        :type accept_failure: boolean
        """
        self.branch = self.deployment.branch
        super(NodeProxy, self).upgrade()
        if not self.has_feature("chefserver"):
            try:
                ...
                if accept_failure:
                    ...
                    raise Exception("chef_-client upgrade failure:{0}".
                                    format(e))

    def apply_feature(self):
        """Runs chef_ client before apply features on node."""
        self.status = "apply-feature"
        if not self.has_feature("chefserver"):
            self.run()
        super(NodeProxy, self).apply_feature()

    def save(self, node=None):
        """Saves a chef_ node to local and remote chef_ server."""
        logger.debug("Saving chef_node:{0}".format(self.name))
        node = node or self.local_node
        node.save(self.local_api)
        if self.remote_api:
            # syncs to remote chef_ server if available
            node.save(self.remote_api)

    def save_locally(self, node=None):
        """Syncs the remote chef_ node attributes to the local chef_ server."""
        logger.debug("Syncing chef_ node from remote:{0}".format(
            self.name))
        if self.remote_api:
    ...
    def run(self, times=1, debug=True, accept_failure=True):
        cmd = monster.util.config['chef_']['client']['run_cmd']
        for i in xrange(times):
            if debug:
                time = self.run_cmd("date +%F_%T")['return'].rstrip()
                log_file = '{0}-client-run.log'.format(time)
                cmd = '{0} -l debug -L "/opt/chef_/{1}"'.format(cmd, log_file)
...
def wrap_node(node, product, environment, deployment, provisioner, branch):
    ...
    chef_remote_node = NodeProxy(name, ip, user, password, product, deployment,
