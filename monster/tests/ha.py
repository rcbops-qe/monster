...
class HATest(Test):
    ...
    def run_tests(self):
        ...
        neutron_net_create_value=tempest.test_node.run_cmd(";".join(["source openrc", "neutron net-create testnet | sed '/^| id/!d' | awk '{{ print $4 }}'"]))
        neutron_net_create_value=neutron_net_create_value['return'].rstrip()
        print "RETVALUE neutron net-create testnet: {0}".format(neutron_net_create_value)


        neutron_subnet_create_value=tempest.test_node.run_cmd(";".join(["source openrc", "neutron subnet-create --name testsubnet {0} 172.32.0.0/24 --no-gateway | sed '/^| id/!d' | awk '{{ print $4 }}'".format(neutron_net_create_value)]))
        neutron_subnet_create_value=neutron_subnet_create_value['return'].rstrip()
        print "RETVALUE neutron subnet-create subtestnet: {0}".format(neutron_subnet_create_value)


        neutron_net_list_value=tempest.test_node.run_cmd(";".join(["source openrc", "neutron net-list | grep {0} | grep {1}".format(neutron_net_create_value, neutron_subnet_create_value)]))
...
#--nic requires the Value for the field "id" from neutron net-create testnet
        nova_boot_value=tempest.test_node.run_cmd(";".join(["source openrc", "nova boot --image cirros-image --flavor 1 --nic net-id={0} testbuild | sed '/^| id/!d' | awk '{{ print $4 }}'".format(neutron_net_create_value)]))
        ...
        while build_status == "BUILD":
            build_status = tempest.test_node.run_cmd(";".join(["source openrc", "nova list | grep {0} | awk '{{ print $6 }}'".format(nova_boot_value)]))
        ...
        neutron_dhcp_value=tempest.test_node.run_cmd(";".join(["source openrc", "neutron dhcp-agent-list-hosting-net testnet | grep {0} | awk '{{ print $6 }}'".format(current_host)]))
        neutron_dhcp_value=neutron_dhcp_value['return'].rstrip()
        print "RETVALUE neutron dhcp-agent-list-hosting-net testnet: {0}".format(neutron_dhcp_value)
